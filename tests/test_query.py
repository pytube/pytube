...
def test_count(cipher_signature):
    ...
    assert cipher_signature.streams.count() == 25


@pytest.mark.parametrize(
    'test_input,expected', [
        ({'progressive': True}, ['18', '36', '17']),
        ...
        ({'subtype': '3gpp'}, ['36', '17']),
        ({'abr': '128kbps'}, ['140', '171']),
        ({'bitrate': '128kbps'}, ['140', '171']),
        ({'audio_codec': 'vorbis'}, ['171']),
...
def test_order_by_descending(cipher_signature):
    ...
    assert mime_types == ['video/mp4', 'video/3gpp', 'video/3gpp']
...
def test_order_by_ascending(cipher_signature):
    ...
    assert mime_types == ['video/3gpp', 'video/3gpp', 'video/mp4']
