...
def test_filesize(cipher_signature):
    assert cipher_signature.streams[0].filesize == 18725717


def test_filesize_kb(cipher_signature):
    assert cipher_signature.streams[0].filesize_kb == float(18286.834)


def test_filesize_mb(cipher_signature):
    assert cipher_signature.streams[0].filesize_mb == float(17.859)


def test_filesize_gb(cipher_signature):
    assert cipher_signature.streams[0].filesize_gb == float(0.018)


def test_filesize_approx(cipher_signature):
    stream = cipher_signature.streams[0]

    assert stream.filesize_approx == 18744571
    stream.bitrate = None
    assert stream.filesize_approx == 18725717
...
def test_description(cipher_signature):
    assert cipher_signature.description.startswith(
        'In 2018, we made something you didn’t like. For Rewind 2019, let’s see what you DID like.'
    )
    assert cipher_signature.description.endswith(
...
def test_thumbnail_when_in_details(cipher_signature):
    expected = f"https://i.ytimg.com/vi/{cipher_signature.video_id}/maxresdefault.jpg"
