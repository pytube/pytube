...
class Video(object):
    ...
    def put_to_file(self, path, chunk_size=8 * 1024, on_progress=None,
                 on_finish=None, force_overwrite=False):
        """Downloads the video.
        ...
        if os.path.isdir(path):
            raise OSError("Can not write into folder:'{0}'".format(path))
        # TODO: Split up the downloading and OS jazz into separate functions.
        response = urlopen(self.url)
        meta_data = dict(response.info().items())
